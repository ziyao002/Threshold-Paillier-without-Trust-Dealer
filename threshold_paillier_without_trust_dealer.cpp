...
NTL::ZZ Paillier_wt::pick_pq(long i = 1){
...
NTL::ZZ Paillier_wt::distributed_RSA_modulus_generation(){
...
Paillier_wt::Paillier_wt(const long input_keyLength = 512) {
     ...
     */
	keyLength = input_keyLength;
	distributed_RSA_modulus_generation();																		// RSA modulus generation
	...
	// verification key
	NTL::ZZ r_vk = Gen_Coprime(N * N);
	vk = NTL::PowerMod(r_vk, 2, N * N);																			// VK = r_vk**2 mod (modulus**2)
	t = 80;																										// t, l: security parameters for ZKP, choose 80 for 
	l = 80;
	
	// secret key distribution
	vk1 = NTL::PowerMod(vk, delta * f1, N * N);																	// VKi is public
	vk2 = NTL::PowerMod(vk, delta * f2, N * N);
	vk3 = NTL::PowerMod(vk, delta * f3, N * N);
...
void Paillier_wt::ZKP_gen_R(NTL::ZZ& c, NTL::ZZ& r, NTL::ZZ& R1, NTL::ZZ& R2){
     ...
     */
    NTL::ZZ Bound = NTL::power(NTL::ZZ(2), t + l) * K * K * K * NTL::power(NTL::ZZ(2), 2 * keyLength);
	r = RandomBnd(Bound);
	R1 = NTL::PowerMod(vk, delta * r, N * N);
	R2 = NTL::PowerMod(c, 4 * delta * r, N * N);
	return;
}

NTL::ZZ Paillier_wt::ZKP_gen_cc(){
...
NTL::ZZ Paillier_wt::ZKP_comput_z(NTL::ZZ& r, NTL::ZZ& cc, NTL::ZZ& fi){
...
bool Paillier_wt::ZKP_check(NTL::ZZ& c, NTL::ZZ& ci, NTL::ZZ& R1, NTL::ZZ& R2, NTL::ZZ& cc, NTL::ZZ& z, NTL::ZZ& vki){
     ...
     */
    NTL::ZZ equality_1_left = NTL::PowerMod(vk, delta * z, N * N);
    NTL::ZZ equality_1_right = NTL::MulMod(R1, NTL::PowerMod(vki, cc, N * N), N * N);
	NTL::ZZ equality_2_left = NTL::PowerMod(c, 4 * delta * z, N * N);
	NTL::ZZ equality_2_right = NTL::MulMod(R2, NTL::PowerMod(ci, 2 * cc, N * N), N * N);
...
bool Paillier_wt::ZKP_for_partial_decryption(NTL::ZZ& ciphertext, NTL::ZZ& c1, NTL::ZZ& c2, NTL::ZZ& c3){
